...
| `documentation_basics`      | Met    | README.md, AGENTS.md (developer guide), docs/README.md (doc map), TOOLS_DOCUMENTATION.md (generated catalogue), CONTRIBUTING.md and SECURITY.md cover installation, usage, contribution and security reporting. |
...
| `repo_interim`        | Met    | Typical contributors open PRs against `main`. Maintainers may push `main` to cut a release (`feat`/`fix`/`perf` publish npm). See `docs/stack/release.md` and `CONTRIBUTING.md`. |
...
| `release_notes`       | Met    | Semantic Release writes notes on the GitHub Release and the npm tarball. Root `CHANGELOG.md` on `main` may lag (`docs/stack/release.md`).                                        |
...
| `vulnerability_report_response` | Met    | SECURITY.md: acknowledge within 6 business days; escalate if no further engagement within 14 days.                                      |
...
| `test`                   | Met    | 440+ unit, integration and property-based tests (`pnpm test`). Coverage gate 70/70/60/70 in `vitest.config.ts`.                                                                                                                        |
...
| `tests_documented_added` | Met    | The policy is documented in CONTRIBUTING.md (Testing) and AGENTS.md (Test Requirements).                                                                                                                                               |
...
| Criterion                        | Status | Justification                                                                                                                                                                                                                                                |
| -------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `know_secure_design`             | Met    | Per-request `AsyncLocalStorage`, monotonic nonces, WRITE confirm preview + stable idempotency, append-only audit, PII-redacted logs, circuit breaker, rate limiter. Product threat model is a local one-user proxy (ADR 0003), not hosted multi-tenant SaaS. |
...
| `crypto_keylength`               | Met    | Bit2Me request signing is HMAC-SHA512 (`src/services/bit2me.ts`). HTTP lockout labels use HMAC-SHA256 with a 256-bit in-memory key (not a tenant id).                                                                                                        |
| `crypto_working`                 | Met    | Only modern, non-deprecated primitives are used (HMAC-SHA256, `crypto.randomBytes`).                                                                                                                                                                         |
| `crypto_pfs`                     | Met    | Outbound Bit2Me API traffic is HTTPS (TLS 1.2+). Local MCP HTTP defaults to loopback. The GitHub Pages homepage is served over HTTPS.                                                                                                                        |
...
| `crypto_random`                  | Met    | `crypto.randomBytes` / `crypto.randomUUID` for correlation ids and idempotency keys.                                                                                                                                                                         |
...
| `english`                         | Met               | Public docs, code comments, commit messages and CI are English. Internal initiative logs (`docs/done-tasks/`, `docs/CHANGELOG.md`) are Spanish; see `docs/README.md`.                                                                                                           |
| `hardening`                       | Met               | Landing CSP / X-Content-Type-Options / Referrer-Policy / Permissions-Policy via `<meta http-equiv>` (GitHub Pages cannot set true HTTP headers). The MCP enforces HTTPS for `BIT2ME_GATEWAY_URL`, redacts PII from logs, and audits WRITE tools. Default HTTP bind is loopback. |
...
The OpenSSF Best Practices badge is already in `README.md`. Re-paste justifications here when the form is next reviewed.
