...
- **Node.js version**: Release CI uses Node 24 (OIDC). Runtime is Node 20+.

### Release Process

Canonical playbook: [`docs/stack/release.md`](./docs/stack/release.md).

1. Push `feat:` / `fix:` / `perf:` to `main` (maintainers). Contributors use a PR.
2. Semantic Release analyzes commits since the last **git tag**
3. Publishes `@bit2me/mcp-server`, creates tag `vX.Y.Z` and a GitHub Release
4. It does **not** commit `package.json` or root `CHANGELOG.md` back to `main` (bot is not a ruleset admin). Trust **npm + the git tag**, not `package.json` on `main`.
...
- **Coverage gate** (Vitest): lines/functions/statements 70%, branches 60% (`vitest.config.ts`). Do not lower the gate. Aspirational line coverage is higher; CI does not require 97%.
...
2. **Release** (`.github/workflows/release.yml`)
    ...
    - Steps: Build, Test, Semantic Release, then Pages (`landing` job)
    - Node.js: **24** (OIDC trusted publishing)
    - Auth to npm: OIDC (no `NPM_TOKEN` in the current workflow)
    - Generates SBOM (CycloneDX) and signs artifacts with cosign keyless signing.
...
2. Setup Node.js 24 with pnpm cache
3. Install dependencies (`pnpm install --frozen-lockfile`)
4. Build (`pnpm build`)
5. Test (`pnpm test`)
6. Semantic Release (OIDC to npm)
7. Redeploy GitHub Pages so the landing sees the new tag

### Required Secrets

- **`GITHUB_TOKEN`**: Auto-provided. npm uses OIDC trusted publishing, not `NPM_TOKEN`.
...
- npm authenticates the **Release** workflow via OIDC (`id-token: write`).
- There is no `NPM_TOKEN` secret in the current `release.yml`.
- `@bit2me/mcp-server` must list that workflow as trusted publisher.

### Publishing Process

See [`docs/stack/release.md`](./docs/stack/release.md). Semantic Release publishes and tags; it does **not** push a version bump commit to `main`.

### Troubleshooting NPM Publishing

- **Error: OIDC / trusted publisher**
    - The Release workflow must stay the trusted publisher for `@bit2me/mcp-server`
    - Job needs `id-token: write`
- **Error: Node version**
    - Release workflow uses Node.js 24 (not 22)
...
1. **Check Node.js version**: Release workflow must stay on Node 24
2. **Check OIDC / trusted publisher** for `@bit2me/mcp-server` (not `NPM_TOKEN`)
3. **Check commit types**: Only `feat:` / `fix:` / `perf:` trigger releases
4. **Verify last tag**: Semantic Release analyzes commits since last tag
5. **Check workflow logs**: Look for authentication errors
6. **`package.json` on `main` looking old is expected** — see [`docs/stack/release.md`](./docs/stack/release.md)
...
### npm publish / OIDC Issues

1. Confirm the Release job has `id-token: write` and `contents: write`
2. Confirm npm trusted publisher matches `.github/workflows/release.yml`
3. Confirm Node 24 in that workflow (OIDC)
4. Do not add `NPM_TOKEN` unless trusted publishing is retired on purpose
