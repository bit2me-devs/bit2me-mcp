...
| `market_get_chart`         | `GET /v3/currency/chart`          | Gets price history (candles/chart) with timestamp, USD price, and Fiat price. Requires ticker pair (e.g., BTC/EUR) and timeframe (one-hour, one-day, one-week, one-month, one-year). Returns last 30 data points with full ISO 8601 date/time and prices in both USD and the fiat currency from the ticker. |
