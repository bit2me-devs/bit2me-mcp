...
## Implementation outline (deferred — requires this ADR's approval)

| Step | File                                  | Change                                                                    |
| ---- | ------------------------------------- | ------------------------------------------------------------------------- |
| 1    | `src/config.ts`                       | Extract `RawConfig` type, add `ConfigResolver` interface.                 |
| 2    | `src/config/env-resolver.ts` (new)    | Wrap the current `dotenv` + env logic.                                    |
| 3    | `src/config/remote-resolver.ts` (new) | Implement URL + shell-command bridges.                                    |
| 4    | `src/config.ts`                       | Replace `getConfig()` body with the chained resolver pipeline.            |
| 5    | `tests/config.test.ts`                | Cover happy-path, last-known-good cache, and explicit-error path.         |
| 6    | `README.md`                           | Document `CONFIG_PROVIDER`, `CONFIG_PROVIDER_URL`, `CONFIG_PROVIDER_CMD`. |
